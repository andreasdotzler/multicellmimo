...
def test_minimax_fixed_fixed(Ms_antennas, Bs_antennas, B, C, H, P, sigma):
    ...
    assert r_d_f_f == pytest.approx(log(det(eye(Bs_antennas) + pinv(Omega)@H.conj().T@Sigma@H)), 1e-3)
    ...
    assert r_d_f_f == pytest.approx(log(det(eye(Bs_antennas) + pinv(Omega_s)@H.conj().T@Sigma_s@H)), 1e-3)
    ...
    assert np.allclose(inv_sqrtm(B)@Sigma_prime@inv_sqrtm(B), Sigma_s, rtol=1e-02, atol=1e-02)
    ...
    X = eye(3) + pinv_sqrtm(Omega_prime) @ Heff.conj().T @ Sigma_prime @ Heff @ pinv_sqrtm(Omega_prime)
    ...
    X = eye(3) + pinv_sqrtm(Omega_prime) @ sqrtm(C) @ H.conj().T @ inv_sqrtm(B) @ Sigma_prime @ inv_sqrtm(B) @ H @ sqrtm(C) @ pinv_sqrtm(Omega_prime)
    #Omega_prime = sqrtm(C) @ Omega_s @ sqrtm(C)
    X = eye(3) + pinv_sqrtm(Omega_prime) @ sqrtm(C) @ H.conj().T @ Sigma_s @ H @ sqrtm(C) @ pinv_sqrtm(Omega_prime)
    X = eye(3) + inv_sqrtm(C) @ sqrtm(C) @ inv_sqrtm((Omega_prime)) @ sqrtm(C) @ H.conj().T @ Sigma_s @ H @ sqrtm(C) @ pinv_sqrtm(Omega_prime)
    X_l = C + sqrtm(C) @ inv_sqrtm(sqrtm(C) @ Omega_s @ sqrtm(C)) @ sqrtm(C) @ H.conj().T @ Sigma_s @ H @ sqrtm(C) @ inv_sqrtm(sqrtm(C) @ Omega_s @ sqrtm(C)) @ sqrtm(C) 
    # TR 2.27 -inv(A) + W + mu I = 0
    Omega_comp = -eye(3) + inv(X)
    Omega_comp = Omega_comp / np.trace(Omega_comp) * np.trace(Omega_prime)
    assert np.allclose(Omega_comp, Omega_prime, rtol=1e-02, atol=1e-02)
    assert np.allclose(inv_sqrtm(C) @ Omega_comp @ inv_sqrtm(C), Omega_s, rtol=1e-02, atol=1e-02)
    ...
    rate_uplink_w_noise = log(det(eye(Bs_antennas) + inv(Omega_prime)@Heff.conj().T @ Sigma_prime @ Heff))
    rate_downlink_w_noise = log(det(eye(Ms_antennas) + Heff @ inv_sqrtm(Omega_prime) @ Q_trans @ inv_sqrtm(Omega_prime) @ Heff.conj().T))
    ...
    Q_downlink = sqrtm(C) @ inv_sqrtm(Omega_prime) @ Q_trans @ inv_sqrtm(Omega_prime) @ sqrtm(C)
...
def test_minimax_opt_opt(Ms_antennas, Bs_antennas, B, C, H, P, sigma):
    ...
    r_d_o_o, R_eff, Q_eff = ptp_worst_case_noise_approx(Heff.conj().T, c*n, p*b, precision=1e-4)
    R = inv_sqrtm(B)@R_eff@inv_sqrtm(B) 
    Q = inv_sqrtm(C)@Q_eff@inv_sqrtm(C) 
    assert r_d_o_o == pytest.approx(log(det(eye(Ms_antennas) + pinv(R)@H@Q@H.conj().T)), 1e-3) 
    ...
    assert r_d_o_o == pytest.approx(log(det(eye(Bs_antennas) + pinv(Omega)@H.conj().T@Sigma@H)), 1e-3)
    ...
    rate_uplink_trans = log(det(eye(Bs_antennas) + H_eff_uplink_2.conj().T @ sqrtm(R) @ B @ sqrtm(R) @ H_eff_uplink_2))
...
def test_minimax_fixed_opt(Ms_antennas, Bs_antennas, B, C, H, P, sigma):
    ...
    Omega = H.conj().T@pinv(R + H@Q@H.conj().T)@H + K
    Sigma = -pinv(R + H @ Q @ H.conj().T) + pinv(R)
    assert r_d_f_o == pytest.approx(log(det(eye(Bs_antennas) + pinv(Omega)@H.conj().T@Sigma@H)), 1e-3)
...
def test_minimax_opt_fixed(Ms_antennas, Bs_antennas, B, C, H, P, sigma):
    ...
    r_d_o_f, R = ptp_worst_case_noise_static(H@ C @H.conj().T, np.trace(B@B), precision=1e-4)
    r_u_o_f, _ = ptp_worst_case_noise_static(H.conj().T @ (p*B) @ H, n*np.trace(C@C), precision=1e-4)
    K = np.zeros(Bs_antennas)
    Omega = H.conj().T@pinv(R + H@C@H.conj().T)@H + K
    Sigma = -pinv(R + H @ C @ H.conj().T) + pinv(R)
    assert r_d_o_f == pytest.approx(log(det(eye(Bs_antennas) + pinv(Omega)@H.conj().T@Sigma@H)), 1e-3)
...
def test_effective_channels(comp, Ms_antennas, Bs_antennas, H, sigma, P, B, C):
    ...
    for i, (Beff, Ceff, Bcon, Ccon) in enumerate([(B, C, I_Ms, I_Bs),  (I_Ms, C, B, I_Bs), (I_Ms, I_Bs, B, C), (B, I_Bs, I_Ms, C)]):
        rel = 1e-2
        Heff = inv_sqrtm(Beff)@H@sqrtm(Ceff)
        rate_d, Q, (Omega_C, _, K) = ptp_capacity_minimax(H=Heff, R=Bcon, C=Ccon, Zs=[np.zeros((Bs_antennas, Bs_antennas))], eps=1e-3)
        Omega = Heff.conj().T@inv(Bcon + Heff@Q@Heff.conj().T)@Heff + K
        assert rate_d == pytest.approx(rate_ref, rel=rel)
        assert log(det(I_Ms + inv(Bcon)@Heff@Q@Heff.conj().T)) == pytest.approx(rate_ref, rel=rel)
        Sigma = -inv(Bcon + Heff@Q@Heff.conj().T) + inv(Bcon)
        Omega_inv = np.linalg.pinv(Omega, rcond=1e-6, hermitian=True)
        rate_u_cal = log(det(I_Bs + Omega_inv*np.trace(Omega@Ccon)/sigma@Heff.conj().T@Sigma@Heff*P/np.trace(Sigma@Bcon)))
        rate_u_cal = log(det(I_Bs + Omega_inv*np.trace(Omega@Ccon)@Heff.conj().T@Sigma@Heff/np.trace(Sigma@Bcon)))
        assert rate_u_cal == pytest.approx(rate_ref, rel=1e-2)
        rate_u, Sigma_eff_BC = ptp_capacity(Heff.conj().T@inv_sqrtm(Bcon), P, Omega/np.trace(Omega@Ccon)*sigma)
        rate_u, Sigma_eff_BC = ptp_capacity(Heff.conj().T@inv_sqrtm(Bcon), 1, Omega/np.trace(Omega@Ccon))
        assert rate_u == pytest.approx(rate_ref, rel=1e-2)


@pytest.mark.parametrize("comp", [0, 1], ids=["real", "complex"])
@pytest.mark.parametrize("H", [np.matrix([[1, 0], [0, 1], [1, 1]]), np.matrix([[1, 0, 0], [1, 0, 0], [0, 0, 1]])], ids=['3x2','3x3'])
