...
from .mimo_worst_case_noise import (
    ...
    ptp_worst_case_noise_unconstraint,
    ptp_worst_case_noise_gradient,
    ptp_worst_case_noise_gradient_projected,
    ptp_worst_case_noise_approx,
    ptp_worst_case_noise_alternating,
...
@pytest.mark.parametrize(
    ...
    ids=["approx"]
    # ids=['gradient', 'projected', 'approx', 'alternating']
)
def test_ptp_worstcase(comp, Ms_antennas, Bs_antennas, f_wc):
    H = (
    ...
    rate_no_channel = log(det(eye(Bs_antennas) + P / sigma * H.conj().T @ H)) / np.log(
        2
    )
    rate_worst_case, (Z, W) = f_wc(H, P, sigma)
    L = sqrtm(Z) @ W @ sqrtm(Z)
